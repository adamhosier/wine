...
import { WINE_REGION_REMOTE_TILE_TEMPLATE, WINE_REGION_TILE_FORMAT, Z_SUBREGION_HI } from "../src/config.js";
...
async function main() {
  ...
  await runPool(tasks, options.concurrency, async ({ z, x, y }) => {
    const outputDir = path.resolve("public", "tiles", "wine-subregions", String(z), String(x));
